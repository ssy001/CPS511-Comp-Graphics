/*******************************************************************
	       Interior Design Modelling Program
********************************************************************/
//////////
#include <windows.h>
#include <gl/gl.h>
...
bool door1closed = true, door2closed = true; // initial state of doors (x2)

// Room Objects
ComplexObj *cobj[12];
//int nextobj = 0; // index to objects array to store next obj
...
void display(void){
  ...
  */
  ...
  if (!door1closed){ // animate door 1 opening/closing using key '1'
  ...
  //if (temp.GetQuaddLength() < 9){
  if (!door2closed){ // animate door 2 opening/closing using key '2'
...
void keyboard(unsigned char key, int x, int y){
  ...
  switch (key) { // Navigate
	...
	case '-':
		...
		deselectAll();
		break;
	case 'b': // activate/deactivate avatar-object collision
		objCollision = !objCollision;
		break;
	case '1':
		door1closed = !door1closed; // open/close door 1
		break;
	case '2': 
		door2closed = !door2closed; // open/close door 2
		break;
	case 'q':
		exit(0);
  }
...
void functionKeys(int key, int x, int y){
  ...
  if (currentAction == NAVIGATE && !personBody->turnHead){
	...
	lookAtz = personBody->translation.z - 2*znew;
	  ...
	  double xzAngle;
	  switch (key){
		  case GLUT_KEY_UP: // 'm' resets head position to 0.0
			  personBody->headAngle.x -= 2.0;
			  if (personBody->headAngle.x < -45) personBody->headAngle.x = -45;
			  break;
		  case GLUT_KEY_DOWN: // 'm' resets head position to 0.0
			  personBody->headAngle.x += 2.0;
